...
from models.modeling_mistral_benchmark_layerwise import MistralForCausalLM as MyMistralForCausalLM
import torch
import transformers
...
def main(args):
    ...
    for data in tqdm(dataset):
        ...
        with torch.inference_mode():
            ...
            for layer_id in range(layer_len):
                response_map = model.model.layers[layer_id].self_attn.prev_attn_typical.sum(dim = 1).max().item()
                features_per_data.append(response_map)
